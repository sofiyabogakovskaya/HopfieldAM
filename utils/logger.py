...
def log_summary(run_id, config: dict, metrics: dict, log_path="experiments/results.log"):
    dataset_name = config.get("dataset_name", 0)
    ...
    line1 = f"{run_id} | dataset_name: {dataset_name} | activation: {activation} | "
    line2 = f"epochs: {epochs} | batch_size: {batch_size} | lr: {lr} | dt: {dt} | t1: {t1} | "
    line3 = f"loss: {loss:.4f} | acc: {acc:.1f}%\n"
    summary_line = line1 + line2 + line3
